      ...
      <div className="flex justify-center space-x-4">
        {plans.map((plan, index) => (
          <Card
            key={index}
            title={plan.title}
            className={`w-[25vw] ${selectedPlan === index ? 'bg-[#18b8b0] text-white' : 'bg-white'} shadow-lg rounded-3xl flex flex-col justify-between`}
            headStyle={{ fontSize: '1.5rem', fontWeight: 'bold' }}
            bodyStyle={{ padding: '1rem', flexGrow: 1, display: 'flex', flexDirection: 'column' }}
            onClick={() => setSelectedPlan(index)}
          >
            <div>
              <div className={`text-4xl font-bold py-6 ${selectedPlan === index ? 'text-white' : 'text-black'}`}>
              ...
              <ul className="list-none mb-6 flex-grow">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className={`mb-2 py-1 ${selectedPlan === index ? 'text-white' : 'text-black'}`}>  <CheckOutlined className="mr-2" /> {feature}</li>
                ))}
              </ul>
            </div>
